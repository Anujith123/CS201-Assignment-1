...
# Q3  :: program to convert an NFA to an equivalent DFA.
...
nfa = {}                                 
n = int(input("Number of states : "))            #Enter total no. of states
t = int(input("Number of transitions : "))       #Enter total no. of transitions/paths eg: a,b so input 2 for a,b,c input 3
for i in range(n):  
    state = input("state name : ")            #Enter state name eg: A, B, C, q1, q2 ..etc
    nfa[state] = {}                           #Creating a nested dictionary
    for j in range(t):
        path = input("path : ")               #Enter path eg : from {a,b} or {0,1}
...
nfa_final_state = [x for x in input().split()]      # Entering final states of NFA
...
dfa = {}                                      #dfa table we needed
keys_list = list(list(nfa.keys())[0])         #conatining all the states in nfa plus the states created in dfa are also appended further
...
print("\nDFA :- \n")    
print(dfa)                                           #Printing the newly created DFA
print("\nPrinting DFA table :- ")
